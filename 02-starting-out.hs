...
isPalindrome' l = and [(l !! i) == (l !! (length l - i - 1)) | i <- [0..(length l - 1)]]
...
{-
 ...
 -}
duplicate xs = concat [n:[n] | n <- xs]
duplicate' xs = [xs !! (i `div` 2) | i <- [0..(2 * length xs)]]
...
dropK k l = take k l ++ drop (k+1) l -- concat [take k l, drop (k+1) l]
...
insertElem x k l = if k < length l then take k l ++ [x] ++ drop k l else error "Index out of bounds"
